...
module agent_market::emergency_exit_tests {
    ...
    fun liquidate_at(
        ...
        let mut vault = scenario.take_shared<UserVault<SUI, TestCrypto>>();
        let clock = scenario.take_shared<Clock>();
        vault_harness::emergency_liquidate_all(
            &mut vault, price_e9, min_fiat_output, NOW_MS + 10_000, &clock, scenario.ctx(),
        );
        test_scenario::return_shared(clock);
        test_scenario::return_shared(vault);
    }
    ...
    fun emergency_liquidation_converts_all_crypto_to_fiat_and_pauses() {
        ...
        liquidate_at(&mut scenario, OWNER, CRASH_PRICE_E9, 100);
    ...
    fun emergency_liquidation_works_while_the_vault_is_paused() {
        ...
        liquidate_at(&mut scenario, OWNER, PRICE_E9, 200);
    ...
    fun agent_cannot_trigger_emergency_liquidation() {
        ...
        liquidate_at(&mut scenario, AGENT, PRICE_E9, 1);
        scenario.end();
    }
    ...
    fun emergency_liquidation_still_enforces_minimum_output() {
        ...
        liquidate_at(&mut scenario, OWNER, CRASH_PRICE_E9, 200);
        scenario.end();
    }
    ...
    fun emergency_liquidation_requires_a_position() {
        let mut scenario = setup();
        liquidate_at(&mut scenario, OWNER, PRICE_E9, 0);
        scenario.end();
    }
